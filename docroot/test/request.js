...
module.exports = (app, chai) => {
  ...
  function request(method, route) {
    return chai.request(app).keepOpen()[method](prefix + '/' + route);
  }
  ...
  return {
     ...
     *   The request.
     */
    auth(username, code = null) {
      if ('object' === typeof username && null === code) {
        code = username.generateTotp();
        username = username.username;
      }

      return request('post', 'user/auth').send({username, code});
    },
    /**
     ...
     *   The request.
     */
    api(auth, method, route) {
      return request(method, route).set('Authorization', auth.body.token_type + ' ' + auth.body.access_token);
    },
  };
};
